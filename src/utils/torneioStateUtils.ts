...
export const torneioStateUtils = {
   ...
   */
// Cole este código dentro do arquivo torneioStateUtils.ts

  /**
   * Updates a specific match result and propagates winner AND loser (for double elimination)
   */
  updateMatchResult: (
    torneios: Torneio[], 
    torneioId: string, 
    ...
    return torneioStateUtils.updateChaveamento(torneios, torneioId, categoriaId, (chaveamento) => {
      const updatedMatches = JSON.parse(JSON.stringify(chaveamento.matches)) as Match[];
      ...
      // 2. Propagate LOSER to lower bracket match (if it exists)
      if (currentMatch.nextMatchIdLoser && currentMatch.nextMatchSlotLoser) {
        const nextMatchLoserIndex = updatedMatches.findIndex(m => m.id === currentMatch.nextMatchIdLoser);
        if (nextMatchLoserIndex !== -1) {
            const slotKey = currentMatch.nextMatchSlotLoser === 1 ? 'a' : 'b';
            updatedMatches[nextMatchLoserIndex] = {
                ...updatedMatches[nextMatchLoserIndex],
                [slotKey]: perdedor
            };
        }
      ...
      const allFinished = finalizadas === updatedMatches.length;
